      ...
      <div className="max-w-md w-full space-y-10 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-3">
            ...
            <div className="relative group">
               <input 
                ...
                required 
                className="w-full h-14 px-5 pr-12 bg-white border border-slate-200 rounded-xl text-slate-900 font-bold text-base
                           focus:border-luxury-gold focus:ring-4 focus:ring-luxury-gold/5 transition-all outline-none"
               />
               <button 
                type="button" 
                onClick={() => setShowPwd(!showPwd)} 
                className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-luxury-gold transition-colors"
               >
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="w-full h-16 bg-slate-900 text-white font-black text-base uppercase tracking-[0.2em] rounded-xl hover:bg-luxury-gold transition-all flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-70 shadow-2xl shadow-slate-900/10 group overflow-hidden relative"
          >
            <span className="relative z-10">{loading ? <Loader2 size={24} className="animate-spin" /> : 'Sign In To Sanctuary'}</span>
        ...
        <div className="relative flex items-center py-6">
          ...
          <span className="flex-shrink mx-6 text-slate-300 text-[10px] font-black uppercase tracking-[0.3em]">Access Via</span>
          <div className="flex-grow border-t border-slate-100"></div>
        </div>
